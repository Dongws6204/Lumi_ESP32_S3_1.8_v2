...
{
    ctx_.packet_buf = static_cast<uint8_t*>(
        heap_caps_malloc(kPacketBufferSize, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
    if (ctx_.packet_buf == nullptr) {
        ctx_.packet_buf = static_cast<uint8_t*>(
            heap_caps_malloc(kPacketBufferSize, MALLOC_CAP_8BIT));
    }
    if (ctx_.packet_buf == nullptr) {
        ESP_LOGE(TAG, "Failed to allocate packet buffer: %zu bytes", kPacketBufferSize);
        return;
    }
    Reset();
}
...
{
    if (ctx_.packet_buf != nullptr) {
        heap_caps_free(ctx_.packet_buf);
        ctx_.packet_buf = nullptr;
    }
}
...
void OggDemuxer::Reset()
{
    ...
    if (ctx_.packet_buf != nullptr) {
        memset(ctx_.packet_buf, 0, kPacketBufferSize);
    }
}
...
{
    if (ctx_.packet_buf == nullptr) {
        ESP_LOGE(TAG, "OggDemuxer packet buffer is not allocated");
        return 0;
    }
        ...
        switch (state_) {
          ...
          case ParseState::PARSE_DATA: {
            while (ctx_.seg_index < ctx_.seg_count && processed < size) {
                ...
                // 检查缓冲区是否足够
                if (ctx_.packet_len + seg_len > kPacketBufferSize) {
                    ESP_LOGE(TAG, "包缓冲区溢出: %zu + %u > %zu", ctx_.packet_len, seg_len, kPacketBufferSize);
                    state_ = ParseState::FIND_PAGE;
                    ctx_.packet_len = 0;
                    ctx_.packet_continued = false;
                    ctx_.seg_remaining = 0;
