...
constexpr int kMaxAudioVolume = 100;
constexpr int kMinAudioVolume = 0;
constexpr int kDefaultAudioVolumeStep = 10;

struct AudioVolumeState {
    std::mutex mutex;
    int current_volume = kDefaultAudioVolume;
    int last_nonzero_volume = kDefaultAudioVolume;
    bool muted = false;
};

AudioVolumeState& GetAudioVolumeState() {
    static AudioVolumeState state;
    return state;
}

int ClampAudioVolume(int volume) { return std::clamp(volume, kMinAudioVolume, kMaxAudioVolume); }

void SyncAudioVolumeStateLocked(AudioVolumeState& state, AudioCodec* codec) {
    int current_volume = ClampAudioVolume(codec->output_volume());
    state.current_volume = current_volume;
    if (current_volume > 0) {
        state.last_nonzero_volume = current_volume;
        state.muted = false;
    }
}
}  // namespace
...
void McpServer::AddCommonTools() {
    ...
    AddTool("self.audio_speaker.set_volume",
            "Deprecated alias of self.audio.volume action=set. Kept for compatibility.",
            PropertyList({Property("volume", kPropertyTypeInteger, 0, 100)}),
            [&board](const PropertyList& properties) -> ReturnValue {
                auto codec = board.GetAudioCodec();
                if (codec == nullptr) {
                    return std::string("Không có audio codec");
                }

                auto& state = GetAudioVolumeState();
                std::lock_guard<std::mutex> lock(state.mutex);
                SyncAudioVolumeStateLocked(state, codec);
                int requested_volume = properties["volume"].value<int>();
                int clamped_volume = ClampAudioVolume(requested_volume);

                codec->SetOutputVolume(clamped_volume);
                state.current_volume = clamped_volume;
                state.muted = clamped_volume == 0;
                if (clamped_volume > 0) {
                    state.last_nonzero_volume = clamped_volume;
                }

                ESP_LOGI(TAG, "[AUDIO] volume set requested=%d clamped=%d", requested_volume,
                         clamped_volume);
                return std::string("Âm lượng đã đặt ") + std::to_string(clamped_volume);
            });
    ...
    AddTool("self.audio.volume",
            ...
            [&board](const PropertyList& properties) -> ReturnValue {
                ...
                if (action == "increase" || action == "decrease") {
                    int step = properties["step"].value<int>();
                    int requested_volume = action == "increase" ? state.current_volume + step
                                                                : state.current_volume - step;
                    int clamped_volume = ClampAudioVolume(requested_volume);
                    codec->SetOutputVolume(clamped_volume);
                    state.current_volume = clamped_volume;
                    state.muted = clamped_volume == 0;
                    ...
                    ESP_LOGI(TAG, "[AUDIO] volume %s requested=%d clamped=%d", action.c_str(),
                             requested_volume, clamped_volume);
                    return std::string("Âm lượng hiện tại ") + std::to_string(clamped_volume);
                }

                if (action == "mute") {
                    if (state.current_volume > 0) {
                        state.last_nonzero_volume = state.current_volume;
                    }
                    codec->SetOutputVolume(0);
                    state.current_volume = 0;
                    state.muted = true;
                    ESP_LOGI(TAG, "[AUDIO] muted last_nonzero=%d", state.last_nonzero_volume);
                    return std::string("Đã tắt tiếng");
                }

                if (action == "unmute") {
                    int restored_volume = state.last_nonzero_volume;
                    if (restored_volume <= 0) {
                        restored_volume = kDefaultAudioVolume;
                    }
                    restored_volume = ClampAudioVolume(restored_volume);
                    codec->SetOutputVolume(restored_volume);
                    state.current_volume = restored_volume;
                    state.last_nonzero_volume = restored_volume;
                    state.muted = false;
                    ESP_LOGI(TAG, "[AUDIO] unmuted volume=%d", restored_volume);
                    return std::string("Đã bật tiếng ") + std::to_string(restored_volume);
                }

                return std::string("Lỗi: action '") + action +
                       "' không hợp lệ. Dùng: set/increase/decrease/mute/unmute";
            });

    AddTool("self.music.play",
            "Phát audio trên loa thiết bị từ một direct HTTPS URL. "
            "URL phải trỏ tới file Ogg chứa Opus mono. "
            "Tool này được sử dụng sau khi cloud MCP tool tìm nhạc trả về "
            "stream_url. Không tự tạo URL và không tìm kiếm bài hát.",
            PropertyList({
            ...
            [](const PropertyList& properties) -> ReturnValue {
                ESP_LOGI(TAG, "[MCP] self.music.play called");
                auto url = properties["url"].value<std::string>();
                auto title = properties["title"].value<std::string>();
                auto volume_value = properties["volume"].provided()
                    ? properties["volume"].value<int>()
                    : 70;

                if (url.empty()) {
                    return std::string("Lỗi: url là bắt buộc");
                }

                auto& app = Application::GetInstance();
                return app.PlayMusic(url, title, volume_value);
            });

    AddTool("self.music.stop",
            "Dừng bài nhạc đang phát trên thiết bị.",
            PropertyList(), [](const PropertyList& properties) -> ReturnValue {
                ESP_LOGI(TAG, "[MCP] self.music.stop called");
                auto& app = Application::GetInstance();
                return app.StopMusic();
            });
